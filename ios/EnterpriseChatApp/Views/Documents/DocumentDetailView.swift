            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            Text(document.icon).font(.largeTitle)
                            Spacer()
                            HStack(spacing: 8) {
                        ...
                        HStack(spacing: 6) {
                            ForEach(document.tags, id: \.self) { tag in
                                Text("#\(tag)")
                                    ...
                                    .overlay {
                                        Capsule().strokeBorder(Color(hex: "6C63FF").opacity(0.2), lineWidth: 0.5)
                                    }
                            }
                        }
                    }
            ...
            ToolbarItem(placement: .topBarTrailing) {
                HStack(spacing: 12) {
                    ...
                    Menu {
                        ...
                        Button { } label: { Label("Export PDF", systemImage: "arrow.down.doc") }
...
struct DocumentBlockView: View {
    ...
    var body: some View {
        switch block.type {
        case .heading1:
            Text(block.content).font(.title2).fontWeight(.bold).padding(.top, 12)
        case .heading2:
            Text(block.content).font(.title3).fontWeight(.semibold).padding(.top, 8)
        case .heading3:
            Text(block.content).font(.headline).padding(.top, 6)
        case .paragraph:
            Text(block.content).font(.body).foregroundStyle(.primary.opacity(0.85)).lineSpacing(4)
        case .bullet:
            HStack(alignment: .top, spacing: 8) {
                Text("•").fontWeight(.bold).foregroundStyle(.secondary)
        ...
        case .numbered:
            HStack(alignment: .top, spacing: 8) {
                Text(block.content.prefix(2).trimmingCharacters(in: .whitespaces))
                    .font(.body).fontWeight(.medium).foregroundStyle(.secondary).frame(width: 24)
                Text(block.content).font(.body)
            }
            .padding(.leading, 8)
        case .todo:
            HStack(spacing: 10) {
                Image(systemName: block.metadata?.isChecked == true ? "checkmark.circle.fill" : "circle")
                    ...
                    .font(.title3)
                    .shadow(color: block.metadata?.isChecked == true ? Color(hex: "34C759").opacity(0.3) : .clear, radius: 3)
                Text(block.content).font(.body)
        ...
        case .callout:
            HStack(alignment: .top, spacing: 10) {
                if let icon = block.metadata?.calloutIcon {
                    Image(systemName: icon).font(.body).foregroundStyle(calloutColor)
                } else {
                    Image(systemName: "info.circle.fill").font(.body).foregroundStyle(calloutColor)
                }
                Text(block.content).font(.subheadline).foregroundStyle(.primary.opacity(0.85))
            }
            .padding(14)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 12)
                    .strokeBorder(calloutColor.opacity(0.25), lineWidth: 0.5)
            }
        case .divider:
        ...
        case .code:
            VStack(alignment: .leading, spacing: 6) {
                if let lang = block.metadata?.language {
                    Text(lang).font(.caption2).fontWeight(.semibold).foregroundStyle(.secondary)
        ...
        case .quote:
            HStack(spacing: 0) {
                RoundedRectangle(cornerRadius: 2)
                    .fill(Color(hex: "6C63FF")).frame(width: 3)
                Text(block.content).font(.body).italic().foregroundStyle(.secondary)
                    .padding(.leading, 12)
            }
            .padding(.leading, 4)
        case .table, .image:
    ...
    private var calloutColor: Color {
        if let hex = block.metadata?.calloutColor { return Color(hex: hex) }
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section("Security") {
                    HStack {
                        Text("Security Level"); Spacer()
                        SecurityBadge(level: document.securityLevel)
                    }
                    LabeledContent("Access") {
                        Label(document.accessLevel.label, systemImage: document.accessLevel.icon).font(.subheadline)
                    }
                }
                Section("Tags") {
                    FlowLayout(spacing: 6) {
                        ForEach(document.tags, id: \.self) { tag in
                            Text("#\(tag)")
                                .font(.caption).foregroundStyle(Color(hex: "6C63FF"))
                                .padding(.horizontal, 10).padding(.vertical, 5)
                                .background(Color(hex: "6C63FF").opacity(0.1), in: Capsule())
                        }
                    }
                }
            }
